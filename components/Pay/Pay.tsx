...
export const Pay: FC<IPayProps> = (props) => {
  ...
  const [wechatH5State, setWechatH5State] = useState<{
    mweb_url: string;
    nonce_str: string;
    prepay_id: string;
    package: string;
  ...
  const onWechatClick = useCallback(async () => {
    try {
      ...
      if (payMethod === PayMethod.scan) {
        ...
        payStatusTimer.current = window.setInterval(
          queryStatus,
          3000,
          order!.id!
        );
    ...
    >
      {!payState.QRCodeVisible && (
        <div>
          <Row className="uranus-row" style={{ rowGap: 0 }}>
            <Col className={styles.item_left} span={8}>
              支付总额
            </Col>
            <Col className={styles.item_right} span={16}>
              ¥ {order.totalPrice! / 100} 元
            </Col>
          </Row>
          <Row className="uranus-row" style={{ rowGap: 0 }}>
            <Col className={styles.item_left} span={8}>
              ...
              <Tooltip
                title={
                  <div>
                    <p className={styles.method_tips}>
                      二维码支付：二维码支付是指用户通过微信、支付宝扫码完成支付的模式
                    </p>
                    <p className={styles.method_tips}>
                      H5支付：H5支付主要是在手机等移动设备中通过浏览器来唤起微信或支付宝支付
                    </p>
                    <p className={styles.method_tips}>
                      微信收银台：在微信浏览器内调用微信支付模块完成支付
                    </p>
                  </div>
                }
              >
            ...
            <Col span={16} className={styles.item_right}>
              <Select
              ...
              >
                ...
                {browserState.os.phone &&
                  !browserState.browser.wechat && ( // 手机端非微信浏览器
                    <Select.Option value={PayMethod.wap}>H5支付</Select.Option>
                  )}
                {browserState.os.phone && ( // 手机端
                  <Select.Option
                    disabled={!browserState.os.phone}
                    value={PayMethod.cashier}
                  >
                    收银台
                  </Select.Option>
                )}
          ...
          <Row
            ...
            className={`uranus-row ${
              payMethod === PayMethod.cashier && !browserState.browser.wechat
                ? styles.hidden
                : ''
            }`}
          >
      ...
      {payState.QRCodeVisible && (
        <div>
            ...
            <Col span={24}>
              <div className={styles.tips}>
                {payState.payType === PayType.WeChatPay && (
                  <>
                    <WechatOutlined className={styles.wechat_pay_icon} />
                    <span style={{ paddingLeft: 8 }}>微信扫码支付</span>
                  </>
                )}
                {payState.payType === PayType.AliPay && (
                  <>
                    <AlipayOutlined className={styles.ali_pay_icon} />
                    <span style={{ paddingLeft: 8 }}>支付宝扫码支付</span>
                  </>
                )}
              </div>
            </Col>
          ...
          <Row style={{ rowGap: 0 }}>
            <Col span={24}>
              <Button type="primary" size="large" block onClick={onGobackClick}>
                返回
              </Button>
            </Col>
          </Row>
      ...
      <form
        action={`https://admin.xunhuweb.com${
          cashierState.type === PayType.WeChatPay
            ? '/pay/cashier'
            : '/alipaycashier'
        }`}
        method="get"
      ...
      >
        ...
        <input
          type="hidden"
          name="out_trade_no"
          value={cashierState.out_trade_no}
        />
        ...
        <input
          type="hidden"
          name="notify_url"
          value={cashierState.notify_url}
        />
        <input
          type="hidden"
          name="redirect_url"
          value={cashierState.redirect_url}
        />
