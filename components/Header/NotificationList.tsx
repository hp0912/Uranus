...
interface INotificationListProps {
  ...
  getData: (
    lastNotiId?: string
  ) => Promise<{ data: { data: INotificationEntity[] } }>;
}
...
export const NotificationList: FC<INotificationListProps> = (props) => {
  ...
  const onLoadMore = useCallback(async () => {
    ...
    const result = await safeProps.current.getData(
      data.length ? data[data.length - 1].id : undefined
    );
  ...
  const onMarkAsReadClick = useCallback(
    async (noti: INotificationEntity) => {
      try {
        setInitLoading(true);
        setLoading(true);
      ...
      {props.type === 'hasnotread' ? (
        <Row className={headerStyles.notification_top} style={{ rowGap: 0 }}>
          ...
          <Col span={8} className={headerStyles.notification_top_01}>
            <Button type="link" onClick={onAllMarkAsReadClick}>
              全部标记为已读
            </Button>
          </Col>
        </Row>
