...
export const CommentEditor: FC<ICommentEditorProps> = (props) => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      userSearch({
        ...
        searchValue: editorState.mentionSearch,
      })
        .then((result) => {
  ...
  const calculatePosition = useCallback(
    (range: Range): { mentionTop: number; mentionLeft: number } => {
      try {
  ...
  const convertToJson = useCallback(
    (content: NodeListOf<ChildNode>): { rows: IUranusNode[][] } => {
          ...
          if (node instanceof HTMLDivElement) {
            ...
            childNode.forEach((cnode) => {
              if (cnode instanceof Text) {
                ...
                contentJSON.rows[rowCount].push({
                  nodeType: IUranusNodeType.text,
                  data: cnode.data,
                });
                ...
                contentJSON.rows[rowCount].push({
                  nodeType: IUranusNodeType.img,
                  attr: { 'data-code': code as string, src: src as string },
                });
              } else if (cnode instanceof HTMLSpanElement) {
                const uid = cnode.getAttribute('data-id');
                contentJSON.rows[rowCount].push({
                  nodeType: IUranusNodeType.span,
                  attr: { 'data-id': uid as string },
                });
              } else if (cnode instanceof HTMLBRElement) {
                contentJSON.rows[rowCount].push({
                  nodeType: IUranusNodeType.br,
                });
              }
            });
  ...
  const onEditorInput = () => {
    ...
    if (selections) {
      ...
      if (lastChar === '@' || mentionSearch !== '') {
        ...
        const newState: ICommentEditorState = Object.assign(
          {},
          editorState,
          position
        );
        ...
        editorState.showEmoji ||
        editorState.showMention ||
        editorState.mentionSearch !== ''
      ) {
        const newState: ICommentEditorState = Object.assign({}, editorState, {
          showEmoji: false,
          showMention: false,
          mentionSearch: '',
        });
      ...
      if (selections1) {
        ...
        if (editorState.mentionSearch) {
          try {
            range.setStart(
              range.endContainer,
              range.startOffset - editorState.mentionSearch.length - 1
            ); // @ + mentionSearch
          } catch {
            //
          }
      ...
      <div className={styles.form_box}>
        ...
        >
          <Spin spinning={mentionUsers.loading}>
            {mentionUsers.users.length > 0 &&
              mentionUsers.users.map((muser) => {
                return (
                  <p
        ...
        <div className={styles.action_box}>
          ...
          <div className={styles.submit}>
            <span style={{ color: '#c2c2c2', marginRight: 8, fontSize: 13 }}>
              Ctrl or ⌘ + Enter
            </span>
            <Button
              type="primary"
