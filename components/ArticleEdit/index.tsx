...
const ArticleEdit: FC<IArticleEditProps> = (props) => {
  ...
  const [state, dispatch] = useReducer<
    (
      preState: IArticleEntity | null,
      action: { type: string; data: IArticleEntity | null }
    ) => IArticleEntity | null
  >(reducer, null);
  ...
  useEffect(() => {
    if (router.query.id === 'new') {
      Promise.all([Promise.resolve<IArticleEntity>(articleInit), tagList()])
      ...
      Promise.all([articleGet(null, router.query.id as string), tagList()])
  ...
  const renderHTML = useCallback<
    (text: string) => HtmlType | Promise<HtmlType> | (() => HtmlType)
  >(
    (text) => {
  ...
  const onChargeChange = useCallback((checked: boolean) => {
    ...
    dispatch({
      type: UPDATEARTICLE,
      data: checked ? { charge: checked } : { charge: checked, amount: 0 },
    });
  }, []);
  ...
  const onEditorChange = useCallback((data: { text: string; html: string }) => {
    unsavedChanges.current = true;
  ...
  const onImageUpload = useCallback(
    async (file: File) => {
      try {
        ...
        const filename = `${AliyunOSSDir}/${
          userContext.userState.id
        }/article_picture_${new Date().getTime()}${suffix}`;
  ...
  const onSaveClick = useCallback(async () => {
    try {
      ...
      if (!coverPicture.length) {
        ...
        coverPicture[0].url &&
        !coverPicture[0].url.match(/\.(?:jpeg|jpg|png|webp|bmp|gif|svg)$/)
      ) {
        ...
        <Skeleton loading={initLoading}>
          ...
          <Row className="uranus-row" style={{ rowGap: 0 }}>
            <Col span={24}>
              ...
              >
                {tags.map((tag) => {
                  return (
                    <Select.Option key={tag.id} value={tag.id as string}>
                      {tag.name}
                    </Select.Option>
                  );
                })}
          ...
          <Row className="uranus-row" style={{ rowGap: 0 }}>
            <Col span={24}>
              ...
              />
              <InputNumber
                className={styles.uranus_article_amount}
                placeholder="付费金额 [元]"
                min={0.01}
                disabled={!state?.charge}
                value={state?.amount}
                precision={2}
